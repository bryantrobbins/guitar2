...
import edu.umd.cs.guitar.testdata.guitar.processor.CoverageProcessor;
import edu.umd.cs.guitar.testdata.guitar.processor.GUIProcessor;
import edu.umd.cs.guitar.testdata.guitar.processor.TestcaseProcessor;
...
public class TestDataManagerTests {
    ...
    private static ArtifactProcessor<TestCase> testCaseArtifactProcessor = new TestcaseProcessor();
    private static ArtifactProcessor<GUIStructure> guiStructureArtifactProcessor = new GUIProcessor();
    private static ArtifactProcessor<ProjectData> serArtifactProcessor = new CoverageProcessor();

    private void addTestCaseFromFiles(TestDataManager tdm, String suiteId, String testId,
                                      String tstPath, String guiPath, String serPath) {

        Map<String, String> options = new HashMap<String, String>();

        try {
            options.put(TestcaseProcessor.FILE_PATH_OPTION, tstPath);
            tdm.addArtifactToTest(testId, options, testCaseArtifactProcessor);
            options.put(TestcaseProcessor.FILE_PATH_OPTION, guiPath);
            tdm.addArtifactToTest(testId, options, guiStructureArtifactProcessor);
            tdm.addArtifactToTest(testId, options, serArtifactProcessor);
        } catch (Exception e) {
            Assert.fail("Caught exception when loading test case from " + tstPath);
        }



        tdm.addTestToSuite(testId, suiteId);
    }
    ...
    public void testAddTestCase() {
        ...
        addTestCaseFromFiles(tdm, "input", testId,
