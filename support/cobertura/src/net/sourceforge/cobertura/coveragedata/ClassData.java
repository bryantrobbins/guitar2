...
/**
 * <p>
 * ProjectData information is typically serialized to a file. An instance of
 * this class records coverage information for a single class that has been
 * instrumented.
 * </p>
 * 
 * <p>
...
public class ClassData extends CoverageDataContainer implements Comparable,
		HasBeenInstrumented {

	private static final String CLASS_SPLITER = "_";
	...
	public LineData addLine(int lineNumber, String methodName,
		...
		try {
			...
			if (lineData == null) {
				...
				// object.
				// Each value is information about the line, stored as a
				// LineData object.
	...
	/**
	 * Returns true if the given object is an instance of the ClassData class,
	 * and it contains the same data as this class.
	 */
	...
	/**
	 * @return The method name and descriptor of each method found in the class
	 *         represented by this instrumentation.
	 */
	...
	public int getNumberOfValidBranches() {
		...
		lock.lock();
		try {
			for (Iterator i = branches.values().iterator(); i.hasNext(); number += ((LineData) i
					.next()).getNumberOfValidBranches())
				;
			return number;
		} finally {
			lock.unlock();
	...
	public int getNumberOfCoveredBranches() {
		...
		try {
			for (Iterator i = branches.values().iterator(); i.hasNext(); number += ((LineData) i
					.next()).getNumberOfCoveredBranches())
				;
			return number;
		} finally {
			lock.unlock();
	...
	/**
	 * Return the name of the file containing this class. If this class'
	 * sourceFileName has not been set (for whatever reason) then this method
	 * will attempt to infer the name of the source file using the class name.
	 * 
	...
	public void merge(CoverageData coverageData) {
		...
		try {
			...
			// super.merge() above.
			for (Iterator iter = classData.branches.keySet().iterator(); iter
					.hasNext();) {
	...
	public void touch(int lineNumber) {
		...
		try {
			...
			TraceManager.collectTraces(sourceFileName, lineNumber);
	 ...
	 * branch on particular line number.
	 * 
	 * @param lineNumber
	 *            The line of code where the branch is
	 * @param branchNumber
	 *            The branch on the line to change the hit counter
	 * @param branch
	 *            The hit counter (true or false)
	 */
	...
	/**
	 * Increments the number of hits for particular hit counter of particular
	 * switch branch on particular line number.
	 * 
	 * @param lineNumber
	 *            The line of code where the branch is
	 * @param switchNumber
	 *            The switch on the line to change the hit counter
	 * @param branch
	 *            The hit counter
	 */
