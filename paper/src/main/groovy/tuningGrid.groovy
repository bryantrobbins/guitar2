...
def dataset = args[0]
def minGammaExp = args[1]
def maxGammaExp = args[2]
def minCostExp = args[3]
def maxCostExp = args[4]
...
for (int gamma=minGammaExp; gamma<=maxGammaExp; i++) {
	for (int cost=minCostExp; cost<=maxCostExp; i++) {
   	...
   	jobParams.put("DATASET", dataset)
   	jobParams.put("GAMMA_EXPONENT", gamma)
   	jobParams.put("COST_EXPONENT", cost)
