...
/**
 * A parser for ARPA LM files which uses an in-memory object rather than a file
 * 
 * @author Bryan Robbins
 */
public class ArpaObjectLmReader<W> implements LmReader<ProbBackoffPair, ArpaLmReaderCallback<ProbBackoffPair>>
{
	...
	protected int currentNGramLength = 1;

	protected int currentNGramCount = 0;
	 ...
	 * @param reader
	 */
	public ArpaObjectLmReader(ArpaObject ao, final WordIndexer<W> wordIndexer) {
		this.ao = ao;
		this.wordIndexer = wordIndexer;
		this.maxOrder = ao.getMaxOrder();
	}
	...
	private void parseNGram(final String string, int start, int stringLength, final int[] retVal) {
		...
		while (true) {
			...
			final String currWord = string.substring(spaceIndex, nextIndex < 0 ? stringLength : nextIndex).trim();
