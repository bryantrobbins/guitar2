...
public class TestDataManager {
	...
	public boolean addTestCaseFromFiles(String dbId, String suiteId,
			String testId, String guitarTestFile, String guitarMapFile,
			String guitarGuiFile, String coberturaSerFile) {

		TestCase theTest = GUITARUtils.getTestCaseFromFile(guitarTestFile);
		GUIMap theMap = GUITARUtils.getMapFromFile(guitarMapFile);
		GUIStructure theGui = GUITARUtils.getGuiFromFile(guitarGuiFile);

		Gson gson = new Gson();
		String jsonMap = gson.toJson(theMap);
		String jsonTest = gson.toJson(theTest);
		String jsonGui = gson.toJson(theGui);

		// Now we need to handle the ser binary file from Cobertura
		// We will use GridFS from Mongo

		File serFileHandle = new File(coberturaSerFile);
		...
		BasicDBObject basicDBObject = new BasicDBObject(
				TestDataManagerDefaults.KEY_TEST_ID, testId)
				.append(TestDataManagerDefaults.KEY_TEST_STEPS, jsonTest)
				.append(TestDataManagerDefaults.KEY_MAP_FILE, jsonMap)
				.append(TestDataManagerDefaults.KEY_GUI_FILE, jsonGui)
				.append(TestDataManagerDefaults.KEY_SER_FILE, serFileId);

		boolean res = true;
		if (suiteId != null) {
			res = addTestToSuite(dbId, testId, suiteId);
		}

		return MongoUtils.addItemToCollection(client.getDB(dbId),
				TestDataManagerDefaults.COLLECTION_TESTS, basicDBObject) && res;
	}
	...
	public TestCase getTestCase(String dbId, String testId) {
		DBCollection tests = client.getDB(dbId).getCollection(
				TestDataManagerDefaults.COLLECTION_TESTS);
		DBObject query = new BasicDBObject(TestDataManagerDefaults.KEY_TEST_ID,
				testId);
		...
		String json = tests.find(query).next().toMap()
				.get(TestDataManagerDefaults.KEY_TEST_STEPS).toString();

		Gson gson = new Gson();
		return gson.fromJson(json, TestCase.class);
	}

    public GUIMap getGUIMap(String dbId, String testId) {
        DBCollection tests = client.getDB(dbId).getCollection(
                TestDataManagerDefaults.COLLECTION_TESTS);
        DBObject query = new BasicDBObject(TestDataManagerDefaults.KEY_TEST_ID,
                testId);
        ...
        String json = tests.find(query).next().toMap()
                .get(TestDataManagerDefaults.KEY_MAP_FILE).toString();

        Gson gson = new Gson();
        return gson.fromJson(json, GUIMap.class);
    }


    public String getCoverageReport(String dbId, String suiteId) {

		String fileId = getSerIdForSuite(dbId, suiteId);
		String ret = null;

		try {
			ProjectData covObject = CoberturaUtils.getCoverageObjectFromGFS(
					client.getDB(dbId), fileId);
			ret = CoberturaUtils.getCoverageReportFromCoverageObject(covObject);
		} catch (IOException e) {
			logger.error("Error while trying to produce coverage report", e);
			ret = null;
		}
	...
	public ProjectData getCoverageObject(String dbId, String suiteId) {
		String fileId = getSerIdForSuite(dbId, suiteId);
		...
		try {
			...
			logger.error("Error retrieving coverage data from GridFS");
			return null;
		}
	...
	public ProjectData getCoverageObjectForTest(String dbId, String testId) {
		DBCollection tests = client.getDB(dbId).getCollection(
				TestDataManagerDefaults.COLLECTION_TESTS);
		DBObject query = new BasicDBObject(TestDataManagerDefaults.KEY_TEST_ID,
				testId);

		String fileId = tests.find(query).next().toMap()
				.get(TestDataManagerDefaults.KEY_SER_FILE).toString();
	...
	private List<TestCase> getTestCasesInSuite(String dbId, String suiteId) {

		List<TestCase> ret = new ArrayList<TestCase>();
		for (String testId : getTestIdsInSuite(dbId, suiteId)) {
			ret.add(getTestCase(dbId, testId));
		}

		return ret;
	}

	private String getSerIdForTest(String dbId, String testId) {
		BasicDBObject query = new BasicDBObject(
				TestDataManagerDefaults.KEY_TEST_ID, testId);

		return client.getDB(dbId)
				.getCollection(TestDataManagerDefaults.COLLECTION_TESTS)
				.findOne(query).toMap()
				.get(TestDataManagerDefaults.KEY_SER_FILE).toString();
	}

	private String getSerIdForSuite(String dbId, String suiteId) {
		BasicDBObject query = new BasicDBObject(
				TestDataManagerDefaults.KEY_SUITE_ID, suiteId);
