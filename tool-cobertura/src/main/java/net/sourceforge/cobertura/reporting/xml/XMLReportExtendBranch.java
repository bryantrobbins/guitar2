...
public class XMLReportExtendBranch {

	private static final Logger logger = Logger
			.getLogger(XMLReportExtendBranch.class);
	...
	public XMLReportExtendBranch(ProjectData projectData, File destinationDir,
		...
		try {
			...
			int numClassesValid = getTotalClasses(projectData);
			int numClassesCovered = getCoveredClasses(projectData);

			int numMethodValid = getTotalMethod(projectData);
			int numMethodCovered = getCoveredMethod(projectData);
			// TODO: Set a schema?
			// println("<coverage " + sourceDirectories.toString() +
			// " xmlns=\"http://cobertura.sourceforge.net\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:schemaLocation=\"http://cobertura.sourceforge.net/xml/coverage.xsd\">");
			println("<coverage line-rate=\""
					...
					+ "\" method-covered=\"" + numMethodCovered
					+ "\" method-valid=\"" + numMethodValid

					+ "\" classes-covered=\"" + numClassesCovered
					+ "\" classes-valid=\"" + numClassesValid
			...
			println("</coverage>");
			System.out.println("======================================");
			System.out.println("Classes covered: " + numClassesCovered);
			System.out.println("Total classes: " + numClassesValid);
			System.out.println();
			System.out.println("Method covered: " + numMethodCovered);
			System.out.println("Total methods: " + numMethodValid);
	...
	private int getCoveredMethod(ProjectData projectData) {

		int numCoveredMethods =0;
		Collection classes = projectData.getClasses();
		Iterator it = classes.iterator();
		while (it.hasNext()) {
			ClassData classData = (ClassData) (it.next());

			int numCoveredLocalMethods = getCoveredMethodInClass(classData);
			numCoveredMethods += numCoveredLocalMethods;
		}
		return numCoveredMethods;
	}

	private int getCoveredMethodInClass(ClassData classData) {
		int numCoveredLocalMethods = 0;
		...
		while (iter.hasNext()) {
			...
			if (lineRate > 0) {
				numCoveredLocalMethods++;
//				System.out.println("DEBUG: " + nameAndSig);
			}

		}
		return numCoveredLocalMethods;
	}

	private boolean isCoveredMethod(ClassData classData, String nameAndSig) {
		Collection lines = classData.getLines(nameAndSig);
		SortedSet sortedLines = new TreeSet();
		sortedLines.addAll(lines);
		Iterator iter = sortedLines.iterator();
		while (iter.hasNext()) {
			LineData line = (LineData) iter.next();
			if (line.isCovered())
				return true;
		}
	 ...
	 * @return
	 */
	private int getTotalMethod(ProjectData projectData) {
		int numTotalMethods = projectData.getClasses().size();
		Collection classes = projectData.getClasses();
		Iterator it = classes.iterator();
		while (it.hasNext()) {
			ClassData classData = (ClassData) (it.next());

			int numTotalLocalMethods = getTotalMethodInClass(classData);
			numTotalMethods += numTotalLocalMethods;
		}
		return numTotalMethods;
	}

	/**
	 * get total method inside a class
	 * 
	 * <p>
	 * 
	 * @param classData
	 * @return
	 */
	private int getTotalMethodInClass(ClassData classData) {
		int numTotalMethods = classData.getMethodNamesAndDescriptors().size();

		return numTotalMethods;
	}

	private int getCoveredClasses(ProjectData projectData) {

		int numClassesCovered = 0;
		Collection classes = projectData.getClasses();
		Iterator it = classes.iterator();
		while (it.hasNext()) {
			ClassData classData = (ClassData) (it.next());
	 ...
	 * @return
	 */
	private int getTotalClasses(ProjectData projectData) {
		int totalClasses = projectData.getClasses().size();
		return totalClasses;
	}
	...
	private void dumpCondition(Object conditionData, String coverage) {
		...
		if (conditionData instanceof JumpData) {
			...
			// Added by baonn@cs.umd.edu
			branches = new long[2];
			branches[0] = jumpData.getFalseHits();
			branches[1] = jumpData.getTrueHits();
			...
			// Added by baonn@cs.umd.edu
			branches = new long[switchData.getNumberOfValidBranches()];
			for (int i = 0;i <branches.length;i++){
				branches[i] = switchData.getHits(i);
			}
			buffer.append(" coverage=\"").append(coverage).append("\">");
	...
	/**
	 * Dump additional branch information
	 * @param branches
	 */
	private void dumpBranches(long[] branches) {
		increaseIndentation();
		
		for (int i = 0;i <branches.length;i++){
			StringBuffer buffer; 
			buffer = new StringBuffer("<branch");
			buffer.append(" number=\"" + i +  "\" " );
			buffer.append(" hits=\"" + branches[i] +  "\" " );
			buffer.append(("/>"));
			println(buffer.toString());
		}
		decreaseIndentation();
